...
// let person = JSON.parse(users);

const Find_user = function(date1, date2){
    let flag = false;
    for (key in users){
        //console.log(key + " -- " + users[key].firstName)
        if ((users[key].registrationDate == date1)||(users[key].registrationDate == date2) ){
            console.log(users[key].firstName + " " + users[key].lastName)
            flag = true;
        }
    }
    if (!flag){
...
Find_user("09.10.2021","10.10.2021");